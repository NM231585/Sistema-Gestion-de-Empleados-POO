...
classDiagram
    ...
    class EmpleadoPorHora {
        ...
        +EmpleadoPorHora(string, string, decimal, decimal)
        +CalcularSalario() decimal
        +ToString() string
    }
    class EmpleadoAsalariado {
        +decimal SueldoMensual
        +EmpleadoAsalariado(string, string, decimal)
        +CalcularSalario() decimal
        +ToString() string
    }
    class EmpleadoComisionista {
        ...
        +EmpleadoComisionista(string, string, decimal, decimal, decimal)
        +CalcularSalario() decimal
        +ToString() string
    }
    class Exception {
...
1. **Clonar el repositorio:**
   ...
   ```
   *(O alternativamente, descarga el archivo ZIP desde GitHub y descomplímelo).*
